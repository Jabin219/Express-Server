...
const setGlobalStopTimeout = (timeoutLimit = sessionLimit) => {
  stopTimeoutId = setTimeout(() => {
    ...
    console.log(
      `${sessionLimit / (60 * 1000)} 分钟完成，休息 ${
        shortRestTime / (60 * 1000)
      } 分钟...`
    );
  }, timeoutLimit);
};
...
const makeFetchRequest = async (
  ...
  while (retryCount <= maxRetries) {
    ...
    try {
      ...
      postSuccessCount++;
      console.log(
        `第 ${postSuccessCount} 次成功的 POST 请求，在第 ${
          retryCount + 1
        } 次尝试成功！响应数据:`,
        data
      );
      ...
      if (error.name === "AbortError") {
        console.error(
          "请求超时！正在调用 /api/parts/latest 检查是否成功插入数据..."
        );

        try {
          const latestResponse = await fetch(
            "http://localhost:8081/api/parts/latest"
          );
          if (!latestResponse.ok) {
            throw new Error(
              `获取最新数据失败，状态码: ${latestResponse.status}`
            );
          }

          const latestData = await latestResponse.json();
          const partsInLatest = latestData.parts || [];

          // 输出对比数据
          console.log("对比字段:", {
            latestPart: partsInLatest,
            inputData: inputData,
          });

          // 检查最新数据中是否包含目标数据
          const isPartInLatest = partsInLatest.some((part) => {
            return (
              part.year === year &&
              part.make === make &&
              part.model === model &&
          ...
          if (isPartInLatest) {
            console.log("目标数据已成功插入数据库，标记为插入成功。");
            postSuccessCount++;
            calculateDropRate();
            isPostInProgress = false; // 请求成功，解除阻塞
            return { success: true, message: "数据已存在于最新数据中。" };
          } else {
            console.warn("目标数据不在最新数据中，插入失败。");
          }
        } catch (latestError) {
          console.error(
            `调用 /api/parts/latest 时发生错误: ${latestError.message}`
          );
        }
      }
      ...
      if (retryCount >= maxRetries) {
        console.error(
          `POST 请求在 ${maxRetries} 次尝试后仍失败。放弃请求。错误信息: ${error.message}`
        );
        postFailCount++;
        ...
        console.warn(
          `POST 请求失败，错误信息: ${error.message}。正在重试第 ${
            retryCount + 1
          } 次 (最多 ${maxRetries} 次)...`
        );
      }
...
const locateLastPosition = async (lastData) => {
  ...
  const retryFindAndClick = async (selector, value, retries = 3) => {
    for (let attempt = 1; attempt <= retries; attempt++) {
      ...
      const triggerSelector = selector.replace(
        "-picker-listEl > *",
        "-trigger-picker"
      );
  ...
  // 定位年份
  const yearElement = await retryFindAndClick(
    "#combo-1060-picker-listEl > *",
    lastData.year
  );
  ...
  // 定位品牌
  const makeElement = await retryFindAndClick(
  ...
  // 定位车型
  const modelElement = await retryFindAndClick(
  ...
  // 定位部件类型
  const typeElement = await retryFindAndClick(
  ...
  const engineElement = await retryFindAndClick(
    ...
    const observer = new MutationObserver((mutations, observer) => {
      ...
      for (let mutation of mutations) {
        if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
          observer.disconnect();
          console.log(
            `Page loaded for ${element.textContent.trim()}. Waiting for 15 seconds...`
          );
...
const fetchAndLocate = async () => {
  try {
    ...
    taskRunCount++; // 增加任务计数
    console.log(
      `开始第 ${taskRunCount} 次 ${runtimeLimit / (60 * 60 * 1000)} 小时任务...`
    );
    ...
    console.log(
      `第 ${taskRunCount} 次 ${
        runtimeLimit / (60 * 60 * 1000)
      } 小时任务完成。将开始 ${restTime / (60 * 60 * 1000)} 小时休息...`
    );
